...
def elemento_fundo():
    s = py5.create_shape(py5.ELLIPSE, 0, 0, DIAMETRO_FUNDO * 1.5, DIAMETRO_FUNDO)
    s.set_stroke(False)
...
def poligono(ratio: float = 3) -> py5.Py5Shape:
    w = TAMANHO
    h = w * ratio
    s = py5.create_shape(py5.RECT, 0, 0, w, h)
    return s
...
def setup():
    ...
    with py5.push_matrix():
        ...
        for angulo in range(0, 360, 36):
            py5.rotate(py5.radians(angulo))
            shuffle(PONTOS)
            for idx, (x, y) in enumerate(PONTOS):
                color = py5.color(abs(180 - idx), 30, 100, 75)
