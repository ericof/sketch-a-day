...
# Copied from: https://codeberg.org/TomLarrow/creative-coding-experiments/src/commit/4c90070b0b6c9eb1519316ffaa00d310d3956f7b/x_0071/PoissonDiscSampling.py
...
class PoissonDiscSampling(object):
    def __init__(
        ...
        field_x2_: int,
        field_y2_: int,
        separation_radius_: float,
        point_size_: float,
        color_: int,
        number_of_retries_: int,
    ):
        ...
        self.separation_radius, self.number_of_retries = (
            separation_radius_,
            number_of_retries_,
        )
        ...
        self.grid_offset_array = np.array(
            [
                [0, -1],
                [1, 0],
                [0, 1],
        ...
        initial_point = py5.Py5Vector2D(
            py5.random(self.field_x1, self.field_x2),
        ...
        while self.active_list:
            active_point = py5.random_choice(self.active_list)
            active_point_location = py5.Py5Vector2D(
                self.point_list[active_point][0], self.point_list[active_point][1]
            )
    ...
    def try_to_add_point(self, point_index: int, point_location: py5.Py5Vector2D):
        for _ in range(self.number_of_retries):
            ...
            random_magnitude = py5.random(
                self.separation_radius, 2 * self.separation_radius
            )
    ...
    def is_valid_point(self, point: py5.Py5Vector2D):
        ...
        if (
            point.x < self.field_x1
            or point.x > self.field_x2
            or point.y < self.field_y1
            or point.y > self.field_y2
        ):
            return False
        ...
        for grid_space in grid_spaces_to_search:
            if (
                0 <= grid_space[0] < self.grid_columns
                and 0 <= grid_space[1] < self.grid_rows
            ):
    ...
    def draw_background(self, background_color: int):
        ...
        py5.rect(
            self.field_x1 - border,
            self.field_y1 - border,
            self.field_x2 + border,
            self.field_y2 + border,
        )
