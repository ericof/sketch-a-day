"""2024-07-20
Memórias 8-bit - Interferência (12)
...
def setup():
    ...
    meio_w = py5.width / 2
    meio_h = py5.height / 2
    for angulo in range(0, 361, 60):
        with py5.push_matrix():
            py5.translate(meio_w, meio_h)
            py5.rotate(py5.radians(angulo))
            for idx, raio in enumerate(range(20, 400, 10)):
                diametro = raio * 2
                if idx % 3 == 1:
                    cor = py5.color(51, 255, 51)
                elif idx % 3 == 2:
                    cor = py5.color(51, 51, 255)
                else:
                    cor = py5.color(255, 176, 0)
                py5.stroke(cor)
                py5.ellipse(-meio_w - 200, -meio_h - 50 - raio, diametro, diametro)
