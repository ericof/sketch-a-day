...
REPETICOES: int = 10
Z_BASE: int = -50
PALETA: str = "op-2363803"


def cmap_da_paleta(nome: str) -> LinearSegmentedColormap:
    """Converte uma paleta do pacote numa colormap contínua do matplotlib.

    Ponto de extensão: :func:`gera_paleta` devolve uma lista discreta de cores,
    enquanto o modo ``CMAP`` do py5 espera uma rampa contínua indexável por um
    valor numérico. Aqui as cores viram os pontos de controle de uma
    interpolação linear, distribuídos uniformemente entre as extremidades.
...
class Particula:
    ...
    def __init__(
        ...
        velocidade=None,
        sentido=None,
        paleta: LinearSegmentedColormap | None = None,
    ):
...
def inicializa():
    ...
    for item in grade:
        ...
        for _ in range(REPETICOES):
            particulas.append(
                Particula(x, y, Z_BASE, diametro=DIAMETRO, paleta=cmap_paleta)
            )
