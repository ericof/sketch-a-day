...
altura: float = 15.0
reducao_topo: float = 0.95
sombra_parede: float = 0.1

noise_generator = OpenSimplex(seed=py5.random_int(20_000))

formas: list[py5.Py5Shape] = []
inclinacao: float = py5.radians(15.0)

largura_ex, altura_ex = helpers.DIMENSOES.external
# Grade em overscan (1.2x) para que a inclinacao da camera nunca revele a borda
# preta do plano; a area extra e centrada em torno do canvas real no draw.
largura_canvas = int(largura_ex * 1.2)
altura_canvas = int(altura_ex * 1.2)
...
def inicializa():
    ...
    grade_ex = cria_grade_ex(
        largura_canvas, altura_canvas, 0, 0, celula_x, celula_y, alternada=alternada
    )
...
def draw():
    ...
    with py5.push():
        # Centra a grade overscan no canvas real e rotaciona em torno do seu
        # centro (nao da borda), para o plano inclinado ficar simetrico nos dois
        # eixos. translate ao centro do canvas -> rotate_x -> volta meio-plano.
        py5.translate(largura_ex / 2, altura_ex / 2, -350)
        py5.rotate_x(inclinacao)
        py5.translate(-largura_canvas / 2, -altura_canvas / 2)
        for forma in formas:
