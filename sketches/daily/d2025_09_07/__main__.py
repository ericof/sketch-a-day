"""2025-09-07
Conexões em PI (08) -- Hexadecimal
Circulo com conexões dos primeiros 1000 dígitos de PI em hexadecimal.
...
def _process_links_digitos(digitos: dict[int, Digito], pi: str, final: int = 3141):
    raw_links = []
    numeros = [d for d in pi if d != "."]
    inicio = 0
    final += inicio - 1
...
def desenha_link(
    ...
    curvatura_maxima = raio * 0.5
    curvatura = min(curvatura_maxima, dist_entre_pontos * 0.5)
...
def setup():
    ...
    pi = txt_helpers.resource_text("pi_hex.txt")
    PALETA = paleta()
...
def desenha(limite: int | None = None):
    ...
    with py5.push():
        ...
        x, y = centro
        y -= 10
        draw_text_box("π", "numero", x, y, font="Georgia-BoldItalic", font_size=60)
    # Credits and go
    canvas.sketch_frame(
