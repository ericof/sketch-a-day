...
FONT_NAME = "SourceCodeProForPowerline-Light"
FONT_SIZE = 16
EXTRA_SPACING = 0.4
MARGIN = 100
CENTER_GLYPH = True
INNER_STEP_SCALE = 0.85
CLOSE_EPS = 1.0
...
def espiral_retangular(
    ...
    while True:
        ...
        place_at = s + (0.5 * w if CENTER_GLYPH else 0.0)
        ...
        with py5.push_matrix():
            py5.translate(px, py)
            py5.rotate(ang)
            py5.text_align(py5.CENTER if CENTER_GLYPH else py5.LEFT, py5.CENTER)
            py5.text(ch, 0, 0)
...
def setup():
    ...
    py5.text_font(py5.create_font(FONT_NAME, FONT_SIZE))
    py5.fill(250)
    texto = txt_helpers.resource_text("ipsum.txt")
    limites = (MARGIN, MARGIN, py5.width - MARGIN, py5.height - MARGIN)
    espiral_retangular(
        texto,
        limites=limites,
