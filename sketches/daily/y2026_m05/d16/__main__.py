"""2026-05-16
Crescimento 04
Grade com círculos concêntricos que nascem a partir de um dos cantos ou do centro.
...
CORES = [
    gera_paleta("mandarin-redux", False),
    gera_paleta("tons-azul-01", False),
    gera_paleta("op-1726494-01", False),
    gera_paleta("op-1726494-01", False),
    gera_paleta("op-1307209", False),
]
...
def calcula_grade():
    ...
    ``QUADRADO`` x ``QUADRADO``, uma paleta sorteada em ``CORES`` (cada
    elemento de ``CORES`` é uma paleta retornada por
    :func:`gera_paleta`), e uma origem compartilhada sorteada em
    ``POS`` (4 cantos da célula + o ponto central). A célula recebe 10
    a 20 círculos concêntricos com tamanho (2 a ``QUADRADO``), cor
    (sorteada da paleta da célula) e traço (1 a 3). Um ``passo`` angular
    é sorteado a cada iteração mas só o do último círculo é armazenado
    por célula — esse valor modula a fase da animação em :func:`draw`
    via ``i * passo``.
    """
