"""2026-05-11
Counting 05 (Chaos 04)
Polígonos regulares a partir de 4 lados, centralizados.
...
def calcula_cor(lado: int, total_células: int) -> list[int]:
    ...
    - ``h = (lado · passo + inicio) mod 360`` com ``passo = 360
      / total_células`` — matiz adapta-se ao tamanho da grade,
      percorrendo o círculo cromático ao longo das células. O
      termo ``inicio`` (constante de módulo) desloca o ponto de
      partida da paleta no círculo cromático;
    ...
    Todas as cores recebem ``alpha`` fixo (constante de módulo),
    permitindo blending por sobreposição entre shapes empilhados.
    ...
    :returns: lista de ``lado`` valores ``py5.color`` com matiz
        comum, variações progressivas de saturação e brilho, e
        alpha fixo.
    """
    passo = 360 / total_células
    h = ((lado * passo) + inicio) % 360
    s_base = 60 + (lado * 7) % 41
    ...
    for i in range(lado):
        ...
        cores.append(py5.color(h, s, b, alpha))
    return cores
