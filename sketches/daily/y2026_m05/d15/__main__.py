"""2026-05-15
Crescimento 04
Grade com círculos concêntricos que nascem a partir do centro de cada célula
...
def calcula_grade():
    ...
    ``QUADRADO`` x ``QUADRADO``, um centro compartilhado (escolhido em
    ``POS``, hoje contendo apenas o ponto central da célula), e 10 a 20
    círculos concêntricos com tamanho (2 a ``QUADRADO``), cor (de
    ``CORES``) e traço (1 a 3). Um ``passo`` angular é sorteado a cada
    iteração mas só o do último círculo é armazenado por célula — esse
    valor modula a fase da animação em :func:`draw` via ``i * passo``.
    """
