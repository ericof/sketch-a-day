...
def constroi_areas() -> list[tuple[float, float, int, list[str | int], int]]:
    ...
    para qualquer ``grade_n``. A celula exatamente no centro -- que so existe em
    grades impares, em ``col == lin == grade_n // 2`` -- recebe ``z_sinal = -1``
    para empilhar na direcao oposta em z; as demais recebem ``1``.
    ...
    return [
        (
            ...
            tamanho_quadrado,
            paleta,
            -1 if grade_n % 2 and col == meio and lin == meio else 1,
        )
