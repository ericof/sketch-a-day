"""2026-07-02
Caquinhos Redux 12
Pisos de caquinhos de São Paulo, grade 4x4.
ericof.com
png
...
def inicializa():
    ...
    # Grade grade_n x grade_n em xadrez (estado/cidade) unida num unico GROUP.
    # Cada celula mede 1/grade_n do painel interno; cada estencil e construido ja
    # escalado e posicionado na sua celula (escala/origem), entao o draw desenha
    # a grade inteira com um so py5.shape().
    cel_x = helpers.DIMENSOES.internal[0] / grade_n
    cel_y = helpers.DIMENSOES.internal[1] / grade_n
    grade_estencil = py5.create_shape(py5.GROUP)
    for lin in range(grade_n):
        for col in range(grade_n):
            arr = arr_estado if (col + lin) % 2 == 0 else arr_cidade
            grade_estencil.add_child(
                estencil_como_forma(
                    arr,
                    cor_fundo,
                    profundidade=profundidade_estencil,
                    escala=cel_x / arr.shape[1],
                    origem=(col * cel_x, lin * cel_y),
                )
            )
...
def key_pressed():
    ...
    global profundidade_estencil, estencil, grade_n
    key = py5.key
    match key:
        case "r":
            inicializa()
