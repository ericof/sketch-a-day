...
jitter_mult: float = 2.0
jitter_bagunca: int = 25
...
def forma_do_recorte(geom) -> py5.Py5Shape | None:
    """Converte a interseccao poligono-celula numa ``Py5Shape`` fechada.
    ...
    maior anel. Retorna ``None`` quando nada sobra (poligono fora da celula).

    :param geom: Geometria shapely resultante do ``intersection``.
    :returns: A forma fechada em coordenadas absolutas, ou ``None`` se vazia.
    """
    if geom.is_empty:
        return None
    ...
    with forma.begin_closed_shape():
        for x, y in maior.exterior.coords[:-1]:
            forma.vertex(x, y)
    return forma
...
def inicializa():
    ...
    for i in range(n_internas):
        ...
        forma = forma_do_recorte(poligono.intersection(celula))
...
def draw():
    ...
    paleta = gera_paleta("bright-colors", True)
    with py5.push():
    ...
    msg = (
        f"escala: {forma_escala:.2f} | "
        f"celulas: {len(formas)} | jitter: {jitter_bagunca}"
    )
    # Credits and go
    canvas.sketch_frame(
        sketch,
