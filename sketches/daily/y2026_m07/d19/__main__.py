...
tamanho_final: int = 60
passo: int = -30
tamanhos = list(range(tamanho_inicial, tamanho_final, passo))
rotacao_inicial: float = 0.0
rotacao_passo: float = 0.3
traco: int = 6

total = len(tamanhos)
h1, s1, b1 = rgb_hex_to_hsb("#f71735")
h2, s2, b2 = rgb_hex_to_hsb("#067bc2")
...
def calcula_cores_traco(idx: int, h: float, s: float, b: float) -> tuple[int, int]:
...
def draw() -> None:
    ...
    with py5.push():
        ...
        desenha_fundo()
    cores: tuple[tuple[float, float, float], ...] = (
        (h1, s1, b1),
        (h2, s2, b2),
    )
    with py5.push():
        ...
        for idx, tamanho in enumerate(tamanhos):
            ...
            h, s, b = cores[idx % 2]
            cor_traco, cor_pintura = calcula_cores_traco(idx, h, s, b)
            if traco:
