...
def inner_ring(d: float, frame: int, radius: int = 1):
...
def middle_ring(e: float, frame: int, point_index: int):
...
def outer_ring(g: float, frame: int, point_index: int, radius: int = 1):
    frame = py5.frame_count
    with py5.push_matrix():
        py5.rotate(g)
        start = cast(
...
def draw_rings():
    ...
    with py5.push():
        ...
        if d < full_turn:
            inner_ring(d, frame, 1)

        if e < full_turn:
            middle_ring(e, frame, point_index)

        if g < full_turn:
            outer_ring(g, frame, point_index, 1)
        else:
            save_and_close()
