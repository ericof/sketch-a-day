...
ESFERA_INFO = [
    (0, 0, -600, [0.0, 90.0, 0.0]),
    (350, 0, -600, [0.0, 90.0, 0.0]),
    (350, 0, -600, [0.0, 90.0, 0.0]),
    (350, 0, -600, [0.0, 90.0, 0.0]),
    (350, 0, -600, [0, 90.0, 0.0]),
    (350, 0, -600, [0.0, 90.0, 0.0]),
    (350, 0, -600, [0, 90.0, 0.0]),
]
...
def setup():
    ...
    with report_time("Cria Esferas"):
        for x, y, z, rotacoes in ESFERA_INFO:
            base = rotacoes[:]
            esfera = cria_esfera(pg, outer_r / 4.0, base)
...
def draw():
    ...
    total = len(esferas) - 1
    incremento = 360 / total
    for idx, info in enumerate(esferas):
        with py5.push():
            py5.translate(*centro)
            py5.rotate(py5.radians((incremento * idx) + 45))
            desenha_olho(info)
    texto = f"Frame: {py5.frame_count} - Luz: ({luz_x}, {luz_y}, {luz_z})"
