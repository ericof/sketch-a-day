...
def cria_esfera(textura: py5.Py5Graphics, rotacoes: list[float]) -> py5.Py5Shape:
    ...
    for meth, angulo in (
        (esfera.rotate_x, rotacoes[0]),
        (esfera.rotate_y, rotacoes[1]),
        (esfera.rotate_z, rotacoes[2]),
    ):
...
def desenha_olho(
    ...
    esfera = cria_esfera(pg, rotacoes)
    with py5.push():
        py5.translate(x, y, z)
        py5.shape(esfera)
...
def draw():
    ...
    with py5.push():
        ...
        if pg:
            for x, y, z, rotacoes in esferas:
                desenha_olho(pg, x, y, z, rotacoes)
    texto = (
