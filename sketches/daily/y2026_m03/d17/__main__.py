...
paleta1: deque[int] = gera_paleta("mondrian", como_deque=True)
paleta2: deque[int] = gera_paleta("italia-01", como_deque=True)
...
def _group_shape(info: InfoGrupo, idx: int):
    """Build and store a py5 group shape from an `InfoGrupo` definition."""
    stroke_weight = 8 / (idx if idx else 1) ** 0.5
    ...
    for shp, _ in formas:
        ...
        poly.set_stroke_weight(stroke_weight)
