...
def inicializa():
    ...
    padroes = [
        *[
            padrao(**payload)
            for padrao in b.Biblioteca.get_categoria("triangulos").values()
        ],
        *[
            padrao(**payload)
            for padrao in b.Biblioteca.get_categoria("circulos").values()
        ],
    ]
    shuffle(padroes)
    posicoes = cria_grade(
        *helpers.DIMENSOES.external, 3, 3, largura_x, largura_y, False
    )
    grade = []
    paleta = deque([
        "#007749",
