...
def setup():
    ...
    paleta: deque[py5.Py5Color] = gera_paleta("laranja-01", True)
    passos = 150
    lx = py5.width
    ly = py5.height // 2
    x = np.linspace(-lx, lx, endpoint=False, num=py5.width * 6)
    y = np.linspace(-ly + 50, ly, endpoint=True, num=passos)
    multiplicadores = np.logspace(1.5, 3.0, num=passos // 2, endpoint=False)
    multiplicadores = sorted(multiplicadores) + sorted(multiplicadores, reverse=True)
    pesos = np.linspace(12, 2, num=passos, endpoint=False)
