...
def camadas_mapa(
    ...
    mapa = mapas.mapa_como_forma(
        ...
        cor_base,
        camadas=[
            ("caminhos", mapas.processa_caminhos(py5.color("#000"), 1)),
            (
                "elementos",
    ...
    if camada_limite := [c.gdf for c in dados.camadas if c.nome == "limites"]:
        ...
        mascara.set_fill(py5.color(20, 20, 20, 95))
        mascara.set_stroke(py5.color("#000"))
...
def setup():
    ...
    cor_base = py5.color("#0000FF")
    tags = {"building": True, "natural": True}
    dados: mapas.GeodataEscalado = mapas.escala_dados(
        mapas.obtem_dados(
            "Miraflores, Lima, Peru", pasta=sketch.path, tags=tags, percentual=40
        ),
    )
