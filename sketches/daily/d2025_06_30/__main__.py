...
def camadas_mapa(
    ...
    if camada_limite := [c.gdf for c in dados.camadas if c.nome == "limites"]:
        ...
        mascara.set_fill(py5.color(0, 0, 0, 99))
        mascara.set_stroke(py5.color(0, 0, 0))
        mascara.set_stroke_weight(4)
        # Ocultar o mapa
        for _ in range(4):
...
def setup():
    ...
    dados: mapas.GeodataEscalado = mapas.escala_dados(
        mapas.obtem_dados("Mooca, São Paulo, São Paulo, Brasil", pasta=sketch.path),
    )
