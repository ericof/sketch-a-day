"""2026-04-09
Language 12
Dodecaedro com textura de grades compostas por padrões de traços e círculos.
ericof.com
png
...
def cria_face_pentagono(
    ...
    # 5 laterais (QUADS) — cor sólida, sem textura.
    laterais = py5.create_shape()
    with laterais.begin_shape(py5.QUADS):
        laterais.no_stroke()
        laterais.fill(celula_fundo)
        for i in range(5):
            j = (i + 1) % 5
            laterais.vertex(
                float(ext[i][0]),
