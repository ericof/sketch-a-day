"""2024-05-25
Grades Sobrepostas III
Criação de três grades, de diferentes tamanhos
png
        ...
        for idy, y in enumerate(range(inicio, final, passo_grade), 1):
            for idx, x in enumerate(range(inicio, final, passo_grade), 1):
                ...
                for _ in range(num_formas):
                    ...
                    cor = choice(CORES)
                    formas.append((pos, tamanho, cor))
                grade.append((pg, (x, y), (idx, idy), fundo, passo, formas, t_quadrado))
...
def draw():
    ...
    with py5.push_matrix():
        ...
        for rot, grade in [(0, GRADE_FUNDO), (15, GRADE), (60, GRADE)]:
            py5.rotate(py5.radians(rot))
            for pg, (x, y), (idx, idy), fundo, passo, formas, t_quadrado in grade:
                pg.begin_draw()
                pg.background(*fundo)
                for forma in formas:
                    (xc, yc), d, cor = forma
