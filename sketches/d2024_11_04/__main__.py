...
def setup():
    ...
    coordenadas = calcula_coordenadas(
        py5.width / 2, py5.height // 2, raio_circulo, passos
    )
    shuffle(coordenadas)
    for idx, (x, y) in enumerate(coordenadas):
        ...
        with py5.push_matrix():
            py5.translate(x, y)
            x0 = py5.sin(9 * x) + py5.sin(x)
            y0 = py5.cos(9 * y) + py5.cos(y)
            py5.circle(0, 0, raio_interno * 2)
            py5.stroke_weight(2)
            py5.stroke("#000")
            py5.circle(x0, y0, raio_interno)
        paleta.rotate(1)
