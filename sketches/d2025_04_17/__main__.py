"""2025-04-17
Padrões Fabricados 07
Fábrica de padrões com paleta de cores diversas
png
...
def gera_colecao() -> list[t.Padrao]:
    ...
    for padrao, peso in [
        ...
        [b.CirculoCentroP(traco=0), 5],
        [b.TrianguloMetades(traco=0), 2],
        [b.TrianguloCanto(traco=0), 20],
...
def setup():
    ...
    paletas = [
        gera_paleta("brasil-01", False),
        gera_paleta("franca-01", False),
        gera_paleta("italia-01", False),
    ]
    rotacoes = range(0, 360, 45)
    with py5.push():
        py5.translate(-200, -200, -10)
