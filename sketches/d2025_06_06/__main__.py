...
def gera_grade(
    ...
    for idx, xb, idy, yb in grade:
        ...
        with pg.begin_draw():
            pg.background(fundo)
            for iidx, diametro in enumerate(range(diametro_max, diametro_min, -10)):
...
def setup():
    ...
    meio_x = py5.width / 2
    meio_y = py5.height / 2
    paleta = gera_paleta("sunset-ocean")
    grade = gera_grade(-meio_x, -meio_y, meio_x, meio_y, 1200, -10, 30, 30, paleta)
