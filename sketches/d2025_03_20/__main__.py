"""2025-03-20
Leaves
Usando curvas bezier, desenhamos uma sequência de formas que se parecem com folhas
png
...
def setup():
    ...
    for idx, xb, idy, yb in cria_grade_ex(
        ...
        x = xb + meio_x
        y = yb + meio_y
        angulo = 60 + idx + idy * 10
        if idy % 2:
            p0 = (0, -meio_y - buffer)
            p1 = (0, meio_y + buffer)
            hb = py5.random_gaussian(180, 20)
        else:
            p0 = (0, meio_y + buffer)
            p1 = (0, -meio_y - buffer)
        if idx % 2:
        ...
        with py5.push():
            ...
            py5.no_fill()
            py5.stroke_weight(1)
            desenha_forma(p0, p1, hb, angulo, celula_x * 1.2, num_passos=40)
