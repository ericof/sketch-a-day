"""2024-04-09
Mosaico VII
Exercício de criação de um mosaico em grade, utilizando quadriláteros.
png
...
def setup():
    ...
    py5.no_stroke()
    for x, y in grade:
        h = py5.random_int(60, 90)
        s = py5.random_int(60, 70)
        b = py5.random_int(40, 70)
        py5.fill(h, s, b)
        s -= 30
        x, y = centro_celula(x, y, largura, altura)
        escala_x = py5.random(0.6, 0.7)
        escala_y = py5.random(0.3, 0.5)
        angulo = py5.random_int(30, 75)
        for modificador in (1, -1):
            forma = py5.create_shape(
                py5.RECT, 0, 0, largura * escala_x, altura * escala_y
            )
