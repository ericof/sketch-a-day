...
def setup():
    ...
    tamanho = 45
    pontos = []
    angulos = range(0, 180, 30)
    for idy, y in enumerate(range(-MARGEM_Y, py5.height + MARGEM_Y, passo)):
        buffer_x = 0 if idy % 2 else tamanho // 4
        for idx, x in enumerate(range(-MARGEM_X, py5.width + MARGEM_X, passo)):
            buffer_y = 0 if idx % 2 else tamanho // 4
            h = (
                (py5.sin(py5.radians(2 * idx)) * 360)
                + (py5.cos(py5.radians(3 * idy)) * 360)
            ) % 360
            forma = formas[idy % 2]
        ...
        for idx, (x, y, forma, h, ang_rot) in enumerate(pontos):
            forma.set_stroke_weight(6)
            cor = py5.color(h, 0, 0)
            forma.set_stroke(cor)
            cor = py5.color(h, 80, 60)
            forma.set_fill(cor)
