...
def quadrilateros(x, y, tamanho, paleta, rotacao=0):
    ...
    tamanhos = np.linspace(tamanho_min, tamanho_max, num=6, endpoint=True)
    x += py5.random_int(-5, 5)
    y += py5.random_int(-10, 10)
...
def setup():
    ...
    grade = cria_grade_ex(py5.width * 2, py5.height * 2, 0, 0, 100, 100, True)
    total = len(grade)
